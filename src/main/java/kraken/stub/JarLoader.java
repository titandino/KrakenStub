...
public class JarLoader {
	 ...
	 * This effectively adds a new JAR to the class path.
	 *
	...
	/**
	 * Returns whether the extending the class path is supported on the host JRE. If
	 * this returns false, the most likely causes are:
	 * <ul>
	 ...
	 * loader (Java &le; 8);
	 * <li>the underlying VM neither supports agents nor uses URLClassLoader as its
	 * system class loader (extremely unlikely from Java 1.6+).
	 * </ul>
	 *
	...
	/**
	 * Returns a string that describes the strategy being used to add JAR files to
	 * the class path. This is meant mainly to assist with debugging and diagnosing
	 * client issues.
	 *
	 * @return returns {@code "none"} if no strategy was found, otherwise a short
	 *         describing the method used; the value {@code "reflection"} indicates
	 *         that a fallback not compatible with Java 9+ is being used
	 */
	 ...
	 * the line {@code Premain-Class: ca.cgjennings.jvm.JarLoader}.
	 *
	 ...
	 * <p>
	 * For this to work the {@code MANIFEST.MF} file <strong>must</strong> include
	 * the line {@code Agent-Class: ca.cgjennings.jvm.JarLoader}.
	 *
	 * @param agentArgs       agent arguments; currently ignored
	 * @param instrumentation provided by the JRE
	 */
	...
	private static Method getAddUrlMethod() {
		if (addUrlMethod == null) {
			...
			if (addUrlThis instanceof URLClassLoader) {
				...
				throw new RuntimeException("did you forget -javaagent:<jarpath>?");
			}
		}
		return addUrlMethod;
	}
